...
# Configuration
st.set_page_config(page_title="App Annotation", page_icon="✏️", layout="wide")
...
def get_next_color():
    color = utils.PREDEFINED_COLORS[
        st.session_state.label_color_index % len(utils.PREDEFINED_COLORS)
    ]
...
def main():
    ...
    if st.session_state.df is None:
        st.markdown(
            "<h1 style='text-align: center;'>Annotation Tool 🚀</h1>", unsafe_allow_html=True
        )
        st.markdown(
            "<p style='text-align: center;'>Charger un fichier pour commencer.</p>",
        ...
        with col_c2:
            ...
            with tab1:
                ...
                if files:
                    selected_file = st.selectbox(
                        "Choisir un fichier", files, label_visibility="collapsed"
                    )
            ...
            with tab2:
                ...
                if upl and st.button("Charger l'upload", use_container_width=True):
    ...
    with st.sidebar:
        st.markdown(
            f"### 📄 {os.path.basename(st.session_state.current_file_path) if st.session_state.current_file_path else 'Fichier temporaire'}"
        )
    ...
    with col_labels:
        ...
        new_label = st.text_input(
            "Nouveau label", label_visibility="collapsed", placeholder="Nouveau label..."
        )
        if (
            st.button("Ajouter", use_container_width=True)
            and new_label
            and new_label not in st.session_state.labels
        ):
    ...
    with col_main:
        ...
        if c1.button("⬅️", use_container_width=True) and st.session_state.current_index > 0:
            save_current_state()
            st.session_state.current_index -= 1
            st.rerun()

        c2.markdown(
            f"<div style='text-align: center; padding-top: 5px;'>Ligne <b>{st.session_state.current_index + 1}</b> / {total_lines}</div>",
            unsafe_allow_html=True,
        )

        if (
            c3.button("➡️", use_container_width=True)
            and st.session_state.current_index < total_lines - 1
        ):
            save_current_state()
            st.session_state.current_index += 1
            st.rerun()
        ...
        try:
            ...
            text_col = (
                "text" if "text" in st.session_state.df.columns else st.session_state.df.columns[0]
            )
        ...
        with st.container(border=True):
            ...
            sel_text = c_sel_text.text_input(
                "Selection", placeholder="Copier-coller le texte...", label_visibility="collapsed"
            )

            opts = list(st.session_state.labels.keys())
            sel_lbl = c_sel_lbl.selectbox(
                "Label", opts if opts else [], label_visibility="collapsed", disabled=not opts
            )
